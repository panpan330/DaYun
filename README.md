...
> 启动三服务后，用客户账号在对话页体验。完整链路如下：

```mermaid
sequenceDiagram
    participant C as 客户（Vue 前端）
    participant A as AI 服务（Python · LangGraph）
    participant R as 知识库（Qdrant RAG）
    participant J as 业务服务（Java · Spring Boot）
    C->>A: 提问"退货政策是什么"
    A->>A: Supervisor 路由 → knowledge Agent
    A->>R: 混合检索（关键词+向量）+ Rerank
    R-->>A: 命中引用片段
    A-->>C: 带引用回答（含来源）
    C->>A: "气死了！三天了还不发货！"
    A->>A: 情绪识别 ANGRY → 生成转人工
    A->>J: 写入转人工队列
    J-->>C: 提示已转人工坐席
    Note over J: 坐席在「工作台 · 转人工」认领/接管
    C->>A: 接管后继续提问
    A->>J: 人工回复回写会话
    J-->>C: 人工回复送达
```
...
- 🧠 **跨会话记忆**：先发一句"我叫小明，孩子上小学"，再开新会话问政策，回复会主动带上称呼与家庭背景。
- 🔍 **思考过程时间线**：对话页下方可折叠"思考过程"，实时展示检索命中哪个知识库、调用哪个工具、用了哪个模型。
- 📊 **运营看板**：登录主管账号查看满意度、转人工量、情绪分布、每日对话量、LLM 成本（按模型/意图聚合）。
